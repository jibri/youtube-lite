 ...
 */
export const getOAuthRedirect = functions.region('europe-west3').https.onRequest((request, response) => {
 ...
 */
export const getOAuthToken = functions.region('europe-west3').https.onRequest(async (request, response) => {
...
export const test = functions.region('europe-west3').https.onRequest(async (request, response) => {
  ...
  response.cookie('redirect', JSON.stringify(redirectCookie), { maxAge: 1000 * 60, httpOnly: true, secure: true })
...
const generateCookie = async (response: functions.Response, email: string, tokens: Credentials) => {
  ...
  await admin.firestore().collection('tokens').doc(rememberme.email).set({ ...tokens, hash: rememberme.hash });
  response.cookie('rememberme', JSON.stringify(rememberme), { maxAge: 1000 * 60 * 60 * 24, httpOnly: true, secure: true })
}
...
const retrieveAndSetToken = async (request: functions.Request, respone: functions.Response): Promise<boolean> => {
  const rememberme = readCookie(request, 'rememberme')
  if (rememberme && rememberme.email) {
    const tokens = (await admin.firestore().collection('tokens').doc(rememberme.email).get()).data() as Creds
